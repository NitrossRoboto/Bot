...
if (CON.LANG == 'SI') ADMİN_USER = '*ඇඩ්මින්වරු ගණන:*', USER_USER = '*සාමාජිකයින් ගණන:*', TR_USER = '*තුර්කි සාමාජිකයින් ගණන:*', Hİ_USER = '*ඉන්දියානු සාමාජිකයින් ගණන:*', AZ_USER = '*Azərbaycan Üzv Sayısı:*', SRİ_USER = '*ශ්‍රීලාංකීය සමාජිකයින් ගණන:*', RU_USER = '*රුසියානු සාමාජිකයින් ගණන:*', USA_USER = '*එක්සත් ජනපද සාමාජිකයින් ගණන:*', OTHER = '*අනෙකුත් රටවල් වල සාමාජිකයින් ගණන:*'
...
if (CON.WORKTYPE == 'private') {
    NitrossBot.addCommand({ pattern: 'whois$', fromMe: true, desc: Lang.PL_DESC }, async (message, match) => { 
    ...
    NitrossBot.addCommand({ pattern: 'whois$', fromMe: false, desc: Lang.PL_DESC }, async (message, match) => { 
        if (message.jid.includes('-')) {
            var json = await message.client.groupMetadataMinimal(message.jid) 
            var code = await message.client.groupInviteCode(message.jid)
            var nwjson = await message.client.groupMetadata(message.jid) 
